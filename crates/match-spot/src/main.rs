...
use match_spot::config::{load_from_path, Config, MqTransport};
use match_spot::health::{spawn_server, BootstrapReady};
use match_spot::mq::{MemoryMessageSource, MemoryOrderSink, MessageSource, OrderSink};
...
async fn main() {
    ...
    let config_path = std::env::var("MATCH_SPOT_CONFIG")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("crates/match-spot/config.example.yaml"));
    ...
    let local_symbols: Option<Vec<String>> =
        std::env::var("MATCH_SPOT_LOCAL_SYMBOLS").ok().map(|s| {
            s.split(',')
                .map(str::trim)
                .filter(|x| !x.is_empty())
                .map(str::to_string)
...
fn build_transport(
    ...
    match config.rocketmq.transport {
        ...
        MqTransport::Rocketmq => Err(
            "RocketMQ transport is not wired yet; set transport: memory for local runs".into(),
        ),
    }
}
